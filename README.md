...
CUBIC C1은 새로운 로봇 프로젝트를 시작할 때마다 차체, 구동부, 전원부, 하위 제어기, 센서 및 ROS 2 환경을 처음부터 구축해야 하는 문제를 줄이기 위해 개발한 **범용 모바일 로봇 기반 플랫폼**이다.
...
| **Troubleshooting** | [`docs/04_Test/`](./docs/04_Test/)                                                                 | 문제 분석 · 원인 · 해결 과정                  |
| **하드웨어 목록**         | [`docs/03_Datasheet/`](./docs/03_Datasheet/)                                                       | 주요 부품 및 하드웨어 구성                     |
| **3D Model**        | [`docs/06_3Dmodel/`](./docs/06_3Dmodel/)                                                           | 전체 조립 및 개별 STEP 모델                  |

> ROS 2 패키지, MCU 펌웨어 및 systemd 서비스의 세부 파일 구성과 사용 방법은 각 디렉터리의 README에서 확인할 수 있다.

---

## 📌 Overview

로봇 프로젝트에서는 실제로 개발하고 싶은 기능보다 그 기능을 구동하기 위한 **이동 플랫폼 자체를 만드는 과정**에 많은 시간과 비용이 들어간다.

직접 제작할 경우 프레임, 모터, 배터리, 전원 회로, 제어기, 배선, 센서 장착 구조와 ROS 2 환경까지 반복적으로 구축해야 한다. 반대로 상용 모바일 플랫폼은 높은 비용이나 폐쇄적인 기구·전기 구조 때문에 새로운 장치와 목적에 맞게 수정하기 어려운 경우가 있다.
...
상세 설계 문서는 상단 바로가기 또는 [`docs/`](./docs) 디렉터리를 참고한다